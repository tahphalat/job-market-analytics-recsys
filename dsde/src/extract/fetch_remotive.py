...
from src.config import RAW_REMOTIVE_DIR, REMOTIVE_URL, SAMPLE_N, SEED
...
def fetch_remotive(api_url: str, out_dir: Path, sample_n: int, seed: int, limit: int | None = None) -> None:
    set_seed(seed)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--api_url", default=REMOTIVE_URL, help="Remotive endpoint URL.")
    parser.add_argument("--out_dir", default=RAW_REMOTIVE_DIR, help="Directory to store outputs.")
    parser.add_argument("--limit", type=int, default=None, help="Optional limit query param.")
...
def main():
    args = parse_args()
    fetch_remotive(args.api_url, Path(args.out_dir), sample_n=args.sample_n, seed=args.seed, limit=args.limit)
